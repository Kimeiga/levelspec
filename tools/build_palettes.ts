...
for (const [id, a] of Object.entries(auto)) {
  ...
  roles.floor = band(roles.floor, 0.28, 0.68);
  roles.static_hard = band(roles.static_hard, 0.34, 0.78);
  roles.exterior_wall = band(roles.exterior_wall, 0.3, 0.74);
  roles.roof = band(roles.roof, 0.16, 0.55);
  for (const k of ['riser', 'stair', 'ramp']) roles[k] = band(roles[k], 0.28, 0.76);
